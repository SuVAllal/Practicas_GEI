/*
 * GEI (ALGORITMOS) - UDC
 * P2 - Ordenacion por insercion y ordenacion shell
 *
...
// Algoritmo 1: ordenacion por insercion
void ord_ins(int v[],int n) {
    int i, x, j;
...
// Test para comprobar el funcionamiento de los algoritmos implementados:
...
void printTablaNLogN(iteration * it, int size_it) { // O(nlogn)
    ...
    printf("%15s %15s %15s %15s %15s\n", "n", "t(n)", "t(n)/n", "t(n)/nlogn", "t(n)/n^1.5");
...
void printTablaShell(iteration * it, int size_it) { // O(n^1.25)
    int i;
    double cotaSubestimada, cotaAjustada, cotaSobrestimada;

    printf("%15s %15s %15s %15s %15s\n", "n", "t(n)", "t(n)/n", "t(n)/1.25", "t(n)/n^1.5");

    for(i = 0; i < size_it; i++) {
        cotaSubestimada = pow(it[i].size, 1);
        cotaAjustada = pow(it[i].size, 1.25);
        cotaSobrestimada = pow(it[i].size, 1.5);
...
int main() {
    iteration * it;
    int size_it = 9; // Rango de n: [500, 128000]

    inicializar_semilla();

    // Validar los tests:
    printf("Test 1: ordenacion por insercion\n");
    test(ord_ins, size_it);
    printf("\n\n");

    printf("Test 2: ordenacion por shell\n");
    test(ord_shell, size_it);
    printf("\n\n");

    // Obtener tablas:
    printf("Ordenacion por insercion: vector aleatorio\n");
    it = getTiempos(ord_ins, aleatorio, size_it);
    printTablaCuadratica(it, size_it);
    printf("\n");


    printf("Ordenacion por insercion: vector descendente\n");
    it = getTiempos(ord_ins, descendente, size_it);
    printTablaCuadratica(it, size_it);
    printf("\n");


    printf("Ordenacion por insercion: vector ascendente\n");
    it = getTiempos(ord_ins, ascendente, size_it);
    printTablaLineal(it, size_it);
    printf("\n");


    printf("Ordenacion por shell: vector aleatorio\n");
    it = getTiempos(ord_shell, aleatorio, size_it);
    printTablaShell(it, size_it);
    printf("\n");


    printf("Ordenacion por shell: vector descendente\n");
    it = getTiempos(ord_shell, descendente, size_it);
    printTablaNLogN(it, size_it);
    printf("\n");


    printf("Ordenacion por shell: vector ascendente\n");
    it = getTiempos(ord_shell, ascendente, size_it);
    printTablaNLogN(it, size_it);
    printf("\n");
