...
-export([comp/1, comp_proc/2, comp_proc/3, decomp/1, decomp_proc/2, decomp_proc/3]).
...
decomp(Archive) ->
    decomp_proc(Archive, string:replace(Archive, ".ch", "", trailing)).

decomp_proc(Archive, Output_File) ->
    decomp_proc(Archive, Output_File, ?DEFAULT_PROCS).
    ...
    case archive:start_archive_reader(Archive) of
        {ok, Reader} ->
            case file_service:start_file_writer(Output_File) of
                {ok, Writer} ->
                    init_decomp_proc(Procs, [], Reader, Writer);
                {error, Reason} ->
                    io:format("Could not open output file: ~w~n", [Reason])
            end;
        {error, Reason} ->
...
init_decomp_proc(Procs, List, Reader, Writer) ->
    case Procs of
        0 ->
            shell_decomp_loop(List, Reader, Writer, self());
        X ->
            Newpid = spawn(?MODULE, decomp_workers_loop, [Reader, Writer, self()]),
            init_decomp_proc(X-1, [Newpid|List],Reader,Writer)
    end.

shell_decomp_loop(PList, Reader, Writer, Shell) -> %%Loop executed by the shell process
    case PList of %%If all of the workers are done the list will be empty
        [] ->
            Reader ! stop,
            Writer ! stop,
        ...
        PList ->
            receive %%If a worker has finished, it shall be removed from the list
                {done,Pid} ->
                    shell_decomp_loop([X || X <- PList,X /= Pid],Reader,Writer, Shell);
                {error,Reason} -> %%Error, all processes should be killed
                    io:format("Error reading input file: ~w~n",[Reason]),
                    [X! stop || X <- PList],
                    Writer! abort,
...
decomp_workers_loop(Reader, Writer, Shell) ->  %% Compression loop => get a chunk, compress it, send to writer
    Reader! {get_chunk, self()},  %% request a chunk from the file reader
    receive
        {chunk, _Num, Offset, Decomp_Data} ->   %% got one, compress and send to writer
            Data = compress:decompress(Decomp_Data),
            Writer! {write_chunk, Offset, Data},
            decomp_workers_loop(Reader, Writer,Shell);
        eof ->  %% end of file, stop and send signal to shell process
            Shell! {done,self()};
        {error, Reason} ->
            Shell! {error, Reason}
