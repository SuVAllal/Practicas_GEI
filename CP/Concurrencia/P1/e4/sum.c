...
struct thread_array {
    struct thread_info *threadIncDec; // Thread para ambos incrementos
...
struct thread_array start_threads(struct options *opt, struct nums *nums, int *t1, int *t2, struct args *argsThreadsInc)
{
    int i;
    struct thread_info *threads, *threadsInc;
    ...
    for (i = 0; i < opt->num_threads; i++) {
        ...
        threadsInc[i].args = &argsThreadsInc[i];
        ...
        // Creamos el nuevo tipo de thread
...
int main (int argc, char **argv)
{
    ...
    struct args argsThreadsInc[opt.num_threads];
    thrs = start_threads(&opt, &nums, &it1, &it2, argsThreadsInc);
    wait(opt, &nums, &thrs);
