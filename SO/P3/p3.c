...
// Cabeceras de las funciones basicas:
void imprimirPrompt();
void leerEntrada(char * input);
void procesarEntrada(char * input, char * tr[], tList *hist, fileList *listaFicheros, tListM * ListMem, tListP * ListProc, char * envp[], char * env[]);
void procesarCadena(char * input, char * tr[], tList *hist, fileList *listaFicheros, tListM * ListMem, tListP * ListProc, char * envp[], char * env[]);
...
void Cmd_command(char * tr[], tList *hist, fileList *listaFicheros, tListM * ListMem, tListP * ListProc, char * envp[], char * env[]);
...
void Cmd_showvar(char * tr[], char * arg3[], char * env[]);
void Cmd_changevar(char * tr[], char * arg3[], char * env[]);
void Cmd_subsvar(char * tr[], char * arg3[], char * env[]);
void Cmd_showenv(char * tr[], char * arg3[], char * env[]);
void Cmd_fork(char * tr[], tListP *ListProc);
void Cmd_exec(char * tr[], char * input, tList * hist, tListM * M, tListP *ListProc, char * envp[]);
void Cmd_jobs(char * tr[], tListP * ListProc);
void Cmd_deljobs(char * tr[], tListP * ListProc);
void Cmd_job(char * tr[], tListP * ListProc);
void Cmd_execLinux(char * tr[], char * input, tListP * ListProc, char * envp[]);
    ...
    if(tr[1] == NULL) {
        ...
        if(p >= 0 && p < last(*hist)) {
            ...
            if(strcmp(tr[0], "command") != 0) {
                ...
                procesarCadena(comando, tr, hist, listaFicheros, ListMem, ListProc, envp, env);
            }
        }
    }
}
...
void Cmd_help(char * tr[]) {
    if(tr[1] == NULL)
        ...
        printf("%s\n", HELP_RECURSE);
    else if(!strcmp(tr[1], "uid"))
        printf("%s\n", HELP_UID);
    else if(!strcmp(tr[1], "showvar"))
        printf("%s\n", HELP_SHOWVAR);
    else if(!strcmp(tr[1], "changevar"))
        printf("%s\n", HELP_CHANGEVAR);
    else if(!strcmp(tr[1], "subsvar"))
        printf("%s\n", HELP_SUBSVAR);
    else if(!strcmp(tr[1], "showenv"))
        printf("%s\n", HELP_SHOWENV);
    else if(!strcmp(tr[1], "fork"))
        printf("%s\n", HELP_FORK);
    else if(!strcmp(tr[1], "exec"))
        printf("%s\n", HELP_EXEC);
    else if(!strcmp(tr[1], "jobs"))
        printf("%s\n", HELP_JOBS);
    else if(!strcmp(tr[1], "deljobs"))
        printf("%s\n", HELP_DELJOBS);
    else if(!strcmp(tr[1], "job"))
        printf("%s\n", HELP_JOB);
...
void Cmd_uid(char * tr[]) {
    ...
    if(tr[1] != NULL && !strcmp(tr[1], "-set") && tr[2] != NULL) {
        if(!strcmp(tr[2], "-l") && tr[3] != NULL)
            ChangeUIDLogin(tr[3]);
        else {
            id = strtol(tr[2], NULL, 10);
...
void Cmd_showvar(char * tr[], char *arg3[], char * env[]) {
    ...
    if(tr[1] == NULL)
        ...
        if((i = BuscarVariable(tr[1], arg3)) == -1) {
            perror("Error al obtener main arg3");
            return;
        }

        if((j = BuscarVariable(tr[1], environ)) == -1) {
            perror("Error al obtener environ");
            return;
        }
...
void Cmd_changevar(char *tr[], char * arg3[], char * env[]) {
    ...
    for(cnt = 0; tr[cnt] != NULL; cnt++);

    if(cnt == 4) {
        var = malloc(strlen(tr[2]) + strlen(tr[3]) + 4);
        strcpy(var, tr[2]);
        strcat(var, "=");
        strcat(var, tr[3]);

        if(!strcmp(tr[1], "-a")) {
            if(CambiarVariable(tr[2], tr[2], tr[3], arg3) == -1) {
                free(var);
            ...
            if(CambiarVariable(tr[2], tr[2], tr[3], env) == -1) {
                free(var);
                perror("Imposible cambiar variable");
                return;
            }
...
void Cmd_subsvar(char * tr[], char * arg3[], char * env[]) {
    ...
    if(cnt == 5) {
        var = malloc(strlen(tr[3]) + strlen(tr[4]) + 2);
        strcpy(var, tr[3]);
        strcat(var, "=");
        strcat(var, tr[4]);

        if(!strcmp(tr[1], "-a")) {
            if(CambiarVariable(tr[2], tr[3], tr[4], arg3) == -1) {
                free(var);
                perror("Imposible cambiar variable");
                return;
            }
            ...
            if(CambiarVariable(tr[2], tr[3], tr[4], env) == -1) {
                free(var);
                perror("Imposible cambiar variable");
                return;
            }
    ...
    if(tr[1] == NULL)
        ...
        if(strstr(tr[n-1], "@") != NULL) {
            if(setpriority(PRIO_PROCESS, getpid(), atoi(tr[cnt] + 1)) == -1) {
                perror("Error de prioridad");
                return;
            }
            tr2[n-1] = NULL;

            if(execvp(tr[1], tr + cnt) == -1) {
...
dataProc updateEstado(dataProc proceso, int opciones) {
    int estado;

   if(opciones != 0)
        opciones = WNOHANG | WUNTRACED | WCONTINUED;

    if(waitpid(proceso.pid, &estado, opciones) == proceso.pid) {
        if(WIFEXITED(estado)) {
            proceso.state = FINISHED;
            proceso.estado = FINISHED;
            proceso.end = WEXITSTATUS(estado);
            ...
            proceso.end = WTERMSIG(estado);
        } else if (WIFSIGNALED(estado)) {
            proceso.state = SIGNALED;
            proceso.estado = SIGNALED;
            proceso.end = WTERMSIG(estado);
        } else {
            proceso.state = UNKNOWN;
            proceso.estado = UNKNOWN;
        }
    }
    return proceso;
}

// Funcion para imprimir un solo proceso
void print_job(dataProc proceso) {
    char * estado;

    if(proceso.estado == STOPPED)
        estado = "PARADO";

    if (proceso.estado == ACTIVE)
        estado = "ACTIVO";

    if (proceso.estado == SIGNALED)
        estado = "SENALADO";

    if(proceso.estado == FINISHED)
        estado = "TERMINADO";

    if(proceso.estado == UNKNOWN)
        estado = "DESCONOCIDO";

    printf("%d\t%s p=%d %s %s (%d) %s\n", proceso.pid, proceso.user, getpriority(PRIO_PROCESS, proceso.pid), proceso.date, estado, proceso.end, proceso.cmd);

}

// Funcion para imprimir todos los procesos
void printJobs(tListP * ListProc) {
    if (isEmptyListP(*ListProc))
        return;
    else {
        int pos;
        dataProc proceso;

        for (pos = firstP(*ListProc); pos <= lastP(*ListProc); pos++) {
            proceso = getItemP(pos, *ListProc);
            proceso = updateEstado(proceso, 1);
            updateItemP(proceso, pos, ListProc);
            print_job(proceso);
        }
    }
}
...
// Borrado del proceso con senal
void delSig(tListP * ListProc) {
    int pos;
    dataProc proceso;
    ...
    if(isEmptyListP(*ListProc))
        ...
        for(pos = firstP(*ListProc); pos <= lastP(*ListProc); pos++) {
            proceso = getItemP(pos, *ListProc);
            if(proceso.estado == SIGNALED)
                deleteItemP(pos, ListProc);
        }
    }
}

// Borrado del proceso terminado
void delTerm(tListP * ListProc) {
    int pos;
    dataProc proceso;
...
// Funcion para encontrar un proceso dado su pid
int findProc(pid_t pid, tListP * ListProc) {
    dataProc proceso;
    int pos;
...
// Funcion para pasar un proceso de segundo plano a primer plano
void fgAux(pid_t pid) {
    int status, sign;
...
void Cmd_job(char * tr[], tListP * ListProc) {
    ...
    if(tr[1] == NULL) {
        Cmd_jobs(tr, ListProc);
        return;
    } else if (!strcmp(tr[1], "-fg") && tr[2] != NULL) {
        pid = (pid_t) strtol(tr[2], NULL, 10);

        if((pos = findProc(pid, ListProc)) != -1) {
            fgAux(pid);
            deleteItemP(pos, ListProc);
        }
    } else {
        ...
        if((pos = findProc(pid, ListProc)) != -1) {
            dataProc proceso = getItemP(pos, *ListProc);
            print_job(proceso);
        } else
            Cmd_jobs(tr, ListProc);
    }
...
/*
 * ***** o ***** &      Ejecucion de programas externos en primer o segundo plano
 */

// Proceso en primer plano
void pplano(char * tr[], bool prioridad, int prio, bool asuser, uid_t user) {
    pid_t pid;

    if((pid = fork()) == 0) {
        if(prioridad) {
            if((setpriority(PRIO_PROCESS, getpid(), prio)) == -1 && errno != 0) {
                perror("Imposible establecer prioridad");
                exit(255);
            }
        }
        ...
        if(asuser) {
            if(setuid(user) == -1) {
                perror("Imposible cambiar credenciales");
                exit(255);
            }
        }
...
void splano(char * tr[], bool prioridad, int prio, bool asuser, uid_t user, tListP * ListProc) {
    dataProc proceso;
    pid_t pid;
    ...
    if((pid = fork()) == 0) {
        ...
        t = time(0);
        strftime(fecha, 128, "%d/%m/%j %H:%M:%S", localtime(&t));
        proceso.date = malloc(strlen(fecha) + 1);
        strcpy(proceso.date, fecha);
        ...
        proceso.user = malloc(strlen(usuario_i)+1);
        strcpy(proceso.user, usuario_i);

        proceso.cmd = malloc(strlen(tr[0]) + 2);
        strcpy(proceso.cmd, tr[0]);
        if(tr[1] != NULL) {
            int i;
            for(i = 1; tr[i] != NULL; i++) {
                strcat(proceso.cmd, " ");
                strcat(proceso.cmd, tr[i]);
            }
        }

        insertItemP(proceso, ListProc);
    }
}
...
void Cmd_execLinux(char * tr[], char * input, tListP * ListProc, char * envp[]) {
    ...
    if(!strcmp(tr[i], "&")) {
        tr[i] = NULL;
        splano(tr, false, 0, false, 0, ListProc);
    } else
        pplano(tr, false, 0, false, 0);
}
...
void procesarEntrada(char * input, char * tr[], tList *hist, fileList *listaFicheros, tListM * ListMem, tListP * ListProc, char * envp[], char * env[]) {
    ...
    procesarCadena(input, tr, hist, listaFicheros, ListMem, ListProc, envp, env);
    ...
    if(tr[0] != NULL) {
        ...
        switch (option(&tr[0])) {
            ...
            case 5:
                ...
                break;
            case 6:
                Cmd_command(tr, hist, listaFicheros, ListMem, ListProc, envp, env);
                break;
            case 7:
                Cmd_listopen(tr, listaFicheros);
                break;
            ...
            case 28:
                ...
                break;
            case 29:
                Cmd_showvar(tr, envp, env);
                break;
            case 30:
                Cmd_changevar(tr, envp, env);
                break;
            case 31:
                Cmd_subsvar(tr, envp, env);
                break;
            case 32:
                Cmd_showenv(tr, envp, env);
                break;
            case 33:
                Cmd_fork(tr, ListProc);
                break;
            case 34:
                Cmd_exec(tr, input, hist, ListMem, ListProc, envp);
                break;
            case 35:
                Cmd_jobs(tr, ListProc);
                break;
            case 36:
                Cmd_deljobs(tr, ListProc);
                break;
            case 37:
                Cmd_job(tr, ListProc);
                break;
            default:
                Cmd_execLinux(tr, input, ListProc, envp);
        }
    }
}
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    while(true) {
        imprimirPrompt();
        leerEntrada(input);
        procesarEntrada(input, tr, &hist, listaFicheros, &ListMem, &ListProc, envp, environ);
    }
}
